...
let print_number number = 
    ...
    let rec print_number' positon =
        let dig_remain = length - positon in
            if(dig_remain >= 69) then
                (printf "%s\\\n%!" (String.sub number positon 69);
                 print_number' (positon + 69))
            else printf "%s\n%!" (String.sub number positon dig_remain)
        in print_number' 0
...
let executereg (thestack: stack_t) (oper: char) (reg: int) =
    try match oper with
        | 'l' -> (
            let entry = Array.get registers reg
            in match entry with
                |false, _ -> printf "register '0%o' is empty\n%!" reg
                |true, num -> push num thestack )
        | 's' -> 
            Array.set registers reg (true,(pop thestack))
